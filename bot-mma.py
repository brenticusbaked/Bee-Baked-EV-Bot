...
# --- API PARAMS ---
SPORT = 'mma_mixed_martial_arts' # UFC and major MMA promotions
REGIONS = 'us,eu'
MARKETS = 'h2h,totals' # Fight winner and Over/Under rounds
                    ...
                    for t in teams:
                        if t in soft:
                            ...
                            if ev > 0.01:
                                ...
                                # MMA Styling
                                header = "🚨 **FIGHT NIGHT EMERGENCY** 🚨" if is_emergency else "🥊 **MMA +EV ALERT** 🥊"
                                picks.append({
                                    "msg": f"{header}\n**Edge:** {ev*100:.2f}%\n**Match:** {matchup}\n**Market:** {m_label} | {t}{pt}\n**Book:** {soft[t]['book']} @ {to_american(s_price)}\n**Suggested:** {units:.2f} Units",
                                    "color": 15158332 if is_emergency else 10038562, # Blood red
...
def main():
    ...
    if picks:
        ...
        requests.post(DISCORD_WEBHOOK_URL, json={"embeds": [{"description": "🥊 **MMA Scan Complete:** No edges found. Bankroll safe. 🛡️", "color": 10038562, "image": {"url": FOOTER_IMG}}]})
