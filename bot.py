...
async def send_discord_alert_async(session, play):
    ...
    embed = {
        ...
        "color": 1500989, 
        "author": { 
            "name": "$BEE BAKED BETS", # <-- Updated!
        },
        "image": {
            "url": "https://pbs.twimg.com/media/HCG2WMSW0AA4zzN?format=jpg&name=900x900"
        },
        "fields": [
        ...
        "footer": {
            "text": "$BEE BAKED BETS Premium Alerts", # <-- Updated!
        },
        "timestamp": datetime.utcnow().isoformat() 
    }
    payload = {"username": "$BEE BAKED BETS", "embeds": [embed]} # <-- Updated!
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if all_profitable_plays:
            ...
            print("No +EV plays found above the 2.0% threshold in this scan.")
            
            payload = {
                "username": "$BEE BAKED BETS", # <-- Updated!
                "content": "🐝 **Scan Complete:** No new +EV edges found right now. $BEE BAKED BETS will check again in 15 minutes!" # <-- Updated!
            }
            try:
                await session.post(DISCORD_WEBHOOK_URL, json=payload)
            except Exception as e:
                print(f"Failed to send empty scan alert: {e}")
