...
REGIONS = 'us,eu,uk' # Added UK for sharper soccer data
MARKETS = 'spreads,totals' # 2-way markets only for perfect math
                    ...
                    for t in teams:
                        if t in soft:
                            ...
                            if ev > 0.01:
                                ...
                                # Global Soccer Styling
                                header = "🚨 **EPL EMERGENCY ALERT** 🚨" if is_emergency else "⚽ **EPL +EV ALERT** ⚽"
                                picks.append({
                                    "msg": f"{header}\n**Edge:** {ev*100:.2f}%\n**Match:** {matchup}\n**Market:** {m_label} | {t}{pt}\n**Book:** {soft[t]['book']} @ {to_american(s_price)}\n**Suggested:** {units:.2f} Units",
                                    "color": 15158332 if is_emergency else 3066993, # Emerald Green for the pitch
...
def main():
    ...
    if picks:
        ...
        requests.post(DISCORD_WEBHOOK_URL, json={"embeds": [{"description": "⚽ **EPL Scan Complete:** No edges found. Bankroll safe. 🛡️", "color": 3066993, "image": {"url": FOOTER_IMG}}]})
