...
# --- API PARAMS ---
SPORT = 'icehockey_nhl' # NHL Targeting
REGIONS = 'us,eu'
# NHL specific markets including Shots on Goal
                    ...
                    for t in teams:
                        if t in soft:
                            ...
                            if ev > 0.01:
                                ...
                                # Updated Header to say NHL with Hockey Emojis
                                header = "🚨 **NHL EMERGENCY ALERT** 🚨" if is_emergency else "🏒 **NHL +EV ALERT** 🏒"
                                picks.append({
                                    "msg": f"{header}\n**Edge:** {ev*100:.2f}%\n**Match:** {matchup}\n**Market:** {m_label} | {t}{pt}\n**Book:** {soft[t]['book']} @ {to_american(s_price)}\n**Suggested:** {units:.2f} Units",
                                    "color": 15158332 if is_emergency else 1146986, # Red for emergency, Dark Blue for NHL
...
def main():
    ...
    if picks:
        ...
        requests.post(DISCORD_WEBHOOK_URL, json={"embeds": [{"description": "🏒 **NHL Scan Complete:** No edges found. Bankroll safe. 🛡️", "color": 1146986, "image": {"url": FOOTER_IMG}}]})
