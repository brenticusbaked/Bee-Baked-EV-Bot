                    ...
                    for t in teams:
                        if t in soft:
                            ...
                            if ev > 0.01:
                                ...
                                header = "🚨 **ESPORTS EMERGENCY** 🚨" if is_emergency else "🎮 **CS2 +EV ALERT** 🎮"
                                picks.append({
                                    "msg": f"{header}\n**Edge:** {ev*100:.2f}%\n**Match:** {matchup}\n**Market:** {m_label} | {t}\n**Book:** {soft[t]['book']} @ {to_american(s_price)}\n**Suggested:** {units:.2f} Units",
                                    "color": 15158332 if is_emergency else 10181046,
...
def main():
    ...
    if picks:
        ...
        requests.post(DISCORD_WEBHOOK_URL, json={"embeds": [{"description": "🎮 **Esports Scan Complete:** No edges found.", "color": 10181046, "image": {"url": FOOTER_IMG}}]})
