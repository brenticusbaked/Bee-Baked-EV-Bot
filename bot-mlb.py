...
# --- API PARAMS ---
SPORT = 'baseball_mlb' # MLB Targeting
REGIONS = 'us,eu'
# MLB specific markets: Pitcher Strikeouts and Batter Home Runs are goldmines
                    ...
                    for t in teams:
                        if t in soft:
                            ...
                            if ev > 0.01:
                                ...
                                m_label = gid.split('_')[0].replace('pitcher_', '').replace('batter_', '').upper()
                                ...
                                # Updated Header to say MLB with Baseball Emojis
                                header = "🚨 **MLB EMERGENCY ALERT** 🚨" if is_emergency else "⚾ **MLB +EV ALERT** ⚾"
                                picks.append({
                                    "msg": f"{header}\n**Edge:** {ev*100:.2f}%\n**Match:** {matchup}\n**Market:** {m_label} | {t}{pt}\n**Book:** {soft[t]['book']} @ {to_american(s_price)}\n**Suggested:** {units:.2f} Units",
                                    "color": 15158332 if is_emergency else 10038562, # Red for emergency, Red-White hue for MLB
...
def main():
    ...
    if picks:
        ...
        requests.post(DISCORD_WEBHOOK_URL, json={"embeds": [{"description": "⚾ **MLB Scan Complete:** No edges found. Bankroll safe. 🛡️", "color": 10038562, "image": {"url": FOOTER_IMG}}]})
