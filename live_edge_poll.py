...
def _send_status(fetch_detail: str, alert_count: int, sent_count: int, cache_sports: int) -> None:
...
def run_live_edge_poll() -> dict:
    ...
    _send_status(fetch_detail, len(alerts), sent, len(current_cache))
    return {
        "detail": f"live edge poll complete | {fetch_detail}",
        "count": sent,
        "label": "alerts",
        "meta": {"alert_candidates": str(len(alerts))},
    }
