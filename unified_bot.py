...
def scan_markets():
    """Analyzes the master cache for +EV opportunities vs Pinnacle."""
    try:
    ...
    for sport, events in cache.items():
        for event in events:
            ...
            # Compare soft books against the sharp (Pinnacle) line
            for m_type, data in markets.items():
                ...
                for s_bet in data['soft']:
                    if s_bet['name'] in sharp:
                        ...
                        # Minimum 3% edge required for alert
                        if s_bet['price'] > p_price * 1.03:
                            ...
                            if not is_already_logged(matchup, m_type, selection):
                                ...
                                log_bet_to_db(
                                    matchup, m_type, selection, 
                                    to_american(s_bet['price']), ev, "1.00", 
                                    to_american(p_price), sport, event['id']
                                )
    ...
    if DISCORD_WEBHOOK_URL and alerts:
        ...
        print(f"Sent {len(alerts)} market alerts.")
