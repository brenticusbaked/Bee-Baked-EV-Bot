...
* `SGO_API_KEY`
* `DISCORD_WEBHOOK_URL`
* `DISCORD_BET_ALERTS_WEBHOOK_URL` (Optional: dedicated `bet-alerts` channel; falls back to `DISCORD_WEBHOOK_URL`)
* `DISCORD_STATUS_WEBHOOK_URL` (Optional: sends workflow-complete status updates to a separate Discord channel)
* `DISCORD_RESULTS_WEBHOOK_URL` (Optional: dedicated `results` channel for grading, CLV, and lifecycle updates; falls back to daily slips/status/default)
...
* `ODDS_PUSH_AUTH_MODE` (Optional: `header` by default; use `query` for ParlayAPI-style `?apiKey=...` auth)
* `ODDS_PUSH_AUTH_HEADER` (Optional: defaults to `X-API-Key`; set to `Authorization` for bearer-token providers)
* `ODDS_PUSH_API_KEY_PARAM` (Optional: defaults to `apiKey` when `ODDS_PUSH_AUTH_MODE=query`)
...
* `LIVE_EDGE_STATUS_NOTIFY` (Default: `false`; sends live-window heartbeat summaries to the status channel)
* `PREGAME_SCAN_WINDOWS` (Default: `90,45,15`)
* `PREGAME_SCAN_TOLERANCE_MINUTES` (Default: `7`)
* `PREGAME_REFRESH_ODDS` (Default: `false`; when true, pregame scans refresh Odds API before scanning)
* `BET_LIFECYCLE_PLAYABLE_EDGE` (Default: `0.005`)
* `BET_LIFECYCLE_MOVEMENT_NOTIFY_PCT` (Default: `1.0`)
* `BET_LIFECYCLE_NOTIFY_INITIAL` (Default: `false`)
* `CLV_SEND_DISCORD_UPDATES` (Default: `true`)
* `CLV_NOTIFY_MIN_CHANGE_PCT` (Default: `0.75`)
* `CLV_MAX_ALERTS` (Default: `10`)
