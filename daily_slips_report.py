...
def build_daily_slips_report() -> str:
    ...
    lifetime_settled = df[df.get("result", "").astype(str).isin(["WIN", "LOSS", "PUSH"])].copy()
    lifetime_wins = int((lifetime_settled.get("result", "").astype(str) == "WIN").sum())
    lifetime_losses = int((lifetime_settled.get("result", "").astype(str) == "LOSS").sum())
    lifetime_pushes = int((lifetime_settled.get("result", "").astype(str) == "PUSH").sum())
    ...
    return (
        ...
        f"Win%: {win_pct:.1f}\n"
        f"ROI: {roi_pct:+.1f}%\n"
        f"Net Units: {net_units:+.2f} (${daily_profit_dollars:+.2f})\n"
        f"CLV Updates: {len(clv_df)}\n"
        f"CLV Beaten: {clv_beaten_rate:.1f}%\n"
        f"Avg CLV Edge: {avg_clv:+.2f}%\n"
        f"Placed Sources: {source_text}\n"
        ...
        f"Lifetime Record: {lifetime_wins}-{lifetime_losses}-{lifetime_pushes}\n"
