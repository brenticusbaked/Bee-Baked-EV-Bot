"""Shared SportsGameOdds (SGO) player-prop scanning engine.

This module factors out the league-agnostic logic that previously lived in
``bot_propodds_nba.py`` so the same Pinnacle-vs-soft-book devig flow can drive
props for any SGO league (NBA, MLB, ...). League-specific stat vocabularies are
supplied through :class:`LeagueConfig`.
"""
...
class LeagueConfig:
    """Configuration for scanning one SGO league's player props."""

    league_id: str  # SGO leagueID, e.g. "NBA" or "MLB"
    sport_key: str  # internal sport key, e.g. "basketball_nba"
    alert_title: str  # Discord alert heading, e.g. "NBA PROP ALERT"
    target_stats: frozenset  # canonical stat names to scan
    stat_aliases: Mapping[str, str]  # alias -> canonical stat name
    stat_labels: Mapping[str, str]  # canonical stat name -> display label
    env_stats_var: str  # env var to override target stats, e.g. "NBA_PROP_STATS"
...
def _normalize_stat_name(value: str, aliases: Mapping[str, str]) -> Optional[str]:
    if not value:
        return None
    key = _slugify(value)
...
def resolve_target_stats(config: LeagueConfig) -> frozenset:
    """Return the active stat set, honouring the league's env override."""
    raw = os.getenv(config.env_stats_var, "")
    if not raw.strip():
        return config.target_stats
    parsed = {
        normalized
        for part in raw.split(",")
        if (normalized := _normalize_stat_name(part, config.stat_aliases))
    }
    return frozenset(parsed) or config.target_stats
...
def _extract_stat(odd_obj: dict, parts: Iterable[str], aliases: Mapping[str, str]) -> Optional[str]:
    candidates: List[str] = []
    parts_list = list(parts)
    if parts_list:
    ...
    for candidate in candidates:
        normalized = _normalize_stat_name(candidate, aliases)
        if normalized:
            return normalized
    return None
...
def get_sgo_edges(config: LeagueConfig):
    if not SGO_API_KEY:
        return [], [], {"reason": "SGO_API_KEY missing"}

    target_stats = resolve_target_stats(config)
    picks = []
    near_misses = []
    book_weights = get_book_weights()
    url = "https://api.sportsgameodds.com/v2/events"
    params = {"apiKey": SGO_API_KEY, "leagueID": config.league_id, "oddsAvailable": "true"}
    scan_stats = {
        "events": 0,
        "raw_odds": 0,
        "parsed_props": 0,
    ...
    try:
        ...
        for event in events_list:
            ...
            for odd_key, odd_obj in odds_iterable:
                ...
                offer = _parse_prop_offer(str(odd_key), odd_obj, target_stats, config.stat_aliases)
                if not offer:
                    continue
                        ...
                        near_misses.append(
                            {
                                ...
                                "edge": edge,
                                "weight": book_weight,
                                "stat": config.stat_labels.get(stat_type, stat_type.upper()),
                            }
                        )
                    if edge < PROP_EV_THRESHOLD:
                        continue

                    market = config.stat_labels.get(stat_type, stat_type.upper())
...
def near_miss_summary(near_misses: List[dict]) -> str:
    if not near_misses:
        return ""
...
def run_prop_bot(config: LeagueConfig, webhook_url: Optional[str] = None) -> dict:
    """Scan one league's props, send Discord alerts, and return a summary dict."""
    if not SGO_API_KEY:
        print(f"SGO_API_KEY missing. Skipping {config.league_id} prop bot.")
        return {"detail": "SGO_API_KEY missing", "count": 0, "label": "alerts"}

    picks, near_misses, scan_stats = get_sgo_edges(config)
    picks = sorted(picks, key=lambda item: item.get("score", 0.0), reverse=True)
    for index, pick in enumerate(picks):
        send_discord_alert(
            {"embeds": [{"description": pick["msg"], "color": 15158332}]},
            source=f"bot_propodds_{config.league_id.lower()}",
    ...
    meta = {}
    summary = near_miss_summary(near_misses)
    if summary:
        meta["near_miss_summary"] = summary
